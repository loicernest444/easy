...
class User_model extends CI_Model {
	...
	public function create_teacher()
	{
		...
		$data['phone'] = html_escape($this->input->post('phone'));
		$data['gender'] = html_escape($this->input->post('gender'));
		$data['birthtype'] = html_escape($this->input->post('bt'));
		$data['birthplace'] = html_escape($this->input->post('place'));
        if ($data['birthtype'] == "on"){
		  $data['birthday'] = strtotime(html_escape($this->input->post('birthday'))); 
        }else{
           $data['birthday'] = html_escape($this->input->post('birthday1'));  
        }
		//$data['blood_group'] = html_escape($this->input->post('blood_group'));
		$data['address'] = html_escape($this->input->post('address'));
		$data['role'] = 'teacher';
		...
		if($duplication_status){
			...
			$teacher_table_data['session'] = $this->active_session;
	...
	public function single_student_create(){ 
		...
		$user_data['birthtype'] = html_escape($this->input->post('bt')) ;
		if($user_data['birthtype'] !='around'){
			$user_data['birthday'] = html_escape($this->input->post('birthday'));
           
		}else{
			$user_data['birthday'] = html_escape($this->input->post('birthday1'));
		}
		
		$user_data['birthplace'] = html_escape($this->input->post('place'));
		$user_data['region_id'] = html_escape($this->input->post('region'));
		$user_data['department'] = html_escape($this->input->post('dep'));
		$user_data['nationality'] = html_escape($this->input->post('nat'));
		$user_data['sporting_ability'] = html_escape($this->input->post('sa'));
		...
		if($duplication_status){
			...
			if ($sec != 'all'){
				...
				$enroll_data['section_id'] = 0;
			}
			$enroll_data['student_status'] = html_escape($this->input->post('student_status'));
			$enroll_data['session'] = $this->active_session;
			$enroll_data['school_id'] = $this->school_id;
			$enroll_data['name'] = $user_data['name'];
			...
			$enroll_data['student_id'] = $student_id;
			$enroll_data['class_id'] = html_escape($this->input->post('class_id'));
			$sec = html_escape($this->input->post('section_id'));
			if ($sec != 'all'){
				$enroll_data['section_id'] = html_escape($this->input->post('section_id'));
			}else{
				$enroll_data['section_id'] = 0;
	...
	public function bulk_student_create(){
		...
		$class_id = html_escape($this->input->post('class_id'));
		$sa = html_escape($this->input->post('sa'));
		$ss = html_escape($this->input->post('ss'));
		$sec = html_escape($this->input->post('section_id'));
		...
		foreach($students_name as $key => $value):
			...
			//if($duplication_status){
				...
				$user_data['birthday'] = $birthday[$key];
				$user_data['sporting_ability'] = $sa[$key];
				...
				$enroll_data['section_id'] = $sec;
				$enroll_data['name'] = $students_name[$key];
				$enroll_data['student_status'] = $ss[$key];
				$enroll_data['session'] = $this->active_session;
				$enroll_data['school_id'] = $this->school_id;
				$this->db->insert('enrols', $enroll_data);
			//}else{
		...
		if (($handle = fopen('uploads/csv_file/student.generate.csv', 'r')) !== FALSE) { // Check the resource is valid
			...
			while (($all_data = fgetcsv($handle, 1000, ",")) !== FALSE) { // Check opening the file is OK!
				if($count > 0){
					...
					$user_data['birthplace'] = html_escape($all_data[2]);
					$ui = html_escape($all_data[3]);
                    if($ui =="M"){
                       $user_data['gender'] = "Male";
                    }else{
                        $user_data['gender'] = "Female";
                    }
					//$user_data['sporting_ability'] = html_escape($all_data[3]);
					$user_data['role'] = $role;
	...
	public function student_singleupdate( ){
		$student_id = html_escape($this->input->post('student_id'));
		$user_id = html_escape($this->input->post('user'));
		$enroll_data['class_id'] = html_escape($this->input->post('class_id'));
		$enroll_data['section_id'] = html_escape($this->input->post('section_id'));
		$enroll_data['name'] = html_escape($this->input->post('name'));
		...
		$user_data['birthplace'] = html_escape($this->input->post('place'));
		$user_data['region_id'] = html_escape($this->input->post('region'));
		$user_data['department'] = html_escape($this->input->post('dep_id'));
		$user_data['nationality'] = html_escape($this->input->post('nat'));
//		$user_data['sporting_ability'] = html_escape($this->input->post('sa'));
		$user_data['gender'] = html_escape($this->input->post('gender'));
//		$user_data['blood_group'] = html_escape($this->input->post('blood_group'));
		...
		if ($duplication_status) {
			...
			$response = array(
				'status' => false,
				'notification' => get_phrase('sorry_this_email_has_been_taken'.$user_data['email'])
			);
		}
		}else{
			$this->db->where('student_id', $student_id);
	...
	public function delete_student($student_id, $user_id) {
		...
		$this->db->where('student_id', $student_id);
		$this->db->delete('average_session');
		$this->db->where('student_id', $student_id);
		$this->db->delete('average_subdivision');
		$this->db->where('student_id', $student_id);
		$this->db->delete('average_division');
		$this->db->where('student_id', $student_id);
 ...
 public function get_school_image($school_id) {
		if (file_exists('uploads/system/school/'.$school_id.'.png'))
		return base_url().'uploads/system/school/'.$school_id.'.png';
		else
		return base_url().'uploads/system/school/default.png';
	}
	...
	public function get_user_image($user_id) {
		...
		return base_url().'uploads/users/placeholder.png';
	}
	// Get User Image Ends
