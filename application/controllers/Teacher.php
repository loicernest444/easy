...
class Teacher extends CI_Controller {
  ...
  if ($action == 'list') {
    ...
    $this->load->view('backend/teacher/option/list', $page_data);
  }
  if ($action == 'lists') {
    $page_data['class_id'] = $id;
    $this->load->view('backend/teacher/option/lists', $page_data);
  }
  // showing the list of expense
  if ($action == 'lists_edit') {
      $page_data['class_id'] = $id;
      $page_data['subject'] = $sub;
      $this->load->view('backend/teacher/option/lists_edit', $page_data);
    }
 ...
 public function section($action = "", $id = "") {
    ...
    if ($action == 'lists') {
      $page_data['class_id'] = $id;
      $this->load->view('backend/teacher/section/lists', $page_data);
    }
    
      // PROVIDE A LIST OF DEPARTMENT ACCORDING TO CLASS ID
    if ($action == 'dep_list') {
      $page_data['dep_id'] = $id;
      $this->load->view('backend/teacher/section/dep_list', $page_data);
    }
  }
	//	SECTION ENDED
    public function class_wise_subject($class_id, $exam_id) {
   ...
   public function class_wise_subjects($class_id, $exam_id) {

    // PROVIDE A LIST OF SUBJECT ACCORDING TO CLASS ID
    $page_data['class_id'] = $class_id;
    $page_data['doing'] = $exam_id;
    $this->load->view('backend/teacher/subject/dropdowns', $page_data);
  } 
    ...
    if($param1 == 'list'){
      ...
      $this->load->view('backend/teacher/marksheet/list', $page_data);
    }
