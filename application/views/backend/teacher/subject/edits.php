...
<?php $subjects = $this->db->get_where('subhead', array('id' => $param1))->result_array(); ?>
...
<form method="POST" class="d-block ajaxForm" action="<?php echo route('subject/update/'.$param1); ?>">
    ...
    <div class="form-group row mb-3" id="suite">
      <?php 
        ...
        $n = $this->db->get_where('subjects', array('class_id' => $subject['class_id'], 'subhead' =>$param1, 'session' => active_session(), 'section_id' =>$uses['id'] ))->num_rows();
            
        $usesub = $this->db->get_where('subjects', array('class_id' => $subject['class_id'], 'subhead' =>$param1, 'session' => active_session(), 'section_id' =>$uses['id'] ))->result_array();
        foreach($usesub as $usesub){
        ?> 
        <table width="100%">
            <thead>
        ...
        <div class="col-md-6">
            ...
            <select name="behavior<?php echo $uses['id']; ?>" id="behavior" class="form-control select2" data-toggle = "select2">
                ...
                <?php foreach($groups as $class){ ?>
                    <option value="<?php echo $class['id']; ?>"<?php if($usesub['behavior'] == $class['id']) echo 'selected'; ?>><?php echo $class['name'].' / '.$class['name_fr']; ?></option>
                <?php } ?>
            </select>
        </div>
        <div class="col-md-6">
            ...
            <select name="group<?php echo $uses['id']; ?>" id="group" class="form-control select2" data-toggle = "select2">
                ...
                <?php foreach($groups as $class){ ?>
                    <option value="<?php echo $class['id']; ?>"<?php if($n > 0) {if($usesub['group'] == $class['id']) echo 'selected';} ?>><?php echo $class['name'].' / '.$class['name_fr']; ?></option>
                <?php } ?>
            </select>
        </div>
        ...
        <div class="col-md-6">
            ...
            <select name="type<?php echo $uses['id']; ?>" id="type<?php echo $uses['id']; ?>" class="form-control select2" data-toggle = "select2">
                ...
                <?php foreach($groups as $class){ ?>
                    <option value="<?php echo $class['id']; ?>"<?php if($n > 0) {if($usesub['type'] == $class['id']) echo 'selected'; }?>><?php echo $class['name'].' / '.$class['name_fr']; ?></option>
                <?php } ?>
            </select>
        </div>
        <div class="col-md-6">
            ...
            <select name="teacher<?php echo $uses['id']; ?>" id="teacher<?php echo $uses['id']; ?>" class="form-control select2" data-toggle = "select2">
                ...
                <?php foreach($groups as $class){ ?>
                    <option value="<?php echo $class['id']; ?>"<?php if($usesub['teacher_id'] == $class['id']) echo 'selected'; ?>><?php echo $this->db->get_where('users', array('id' => $class['user_id']))->row()->name; ?></option>
                <?php } ?>
            </select>
        </div>
        <div class="form-group col-md-6">
          <label for="name"><?php echo get_phrase('coef'); ?></label>
            ...
            <input type="hidden" class="form-control" id="subh" name="subh<?php echo $uses['id']; ?>" value="<?php echo $param1; ?>">
          <input type="number" min="0" step="0.01" class="form-control" value="<?php if($n > 0) {echo $usesub['coef'];} ?>" id="coef<?php echo $uses['id']; ?>" name="coef<?php echo $uses['id']; ?>">
        </div>
        <div class="col-md-12">
