...
$enrolss = $this->crud_model->get_div_averages($class_id, $section_id, $exam_id)->result_array();
        ...
        <?php 
            ...
            <tr>
                    ...
                    <?php
                      foreach($exs as $bids){
                        ...
                        $id = $this->crud_model->get_idmarks_average($mark['class_id'], $mark['section_id'], $sub['id'], $bids['id'], $mark['student_id']);
                            if($co == 1){ ?>
                                <td  style="border : 1px solid black; color: #000; text-align : center;" <?php if($this->crud_model->get_stmarks_average($mark['class_id'], $mark['section_id'], $sub['id'], $bids['id'], $mark['student_id'])->row()->mark_obtained < 10) { ?> style="color:red;" <?php } ?>><?php echo $this->crud_model->get_stmarks_average($mark['class_id'], $mark['section_id'], $sub['id'], $bids['id'], $mark['student_id'])->row()->mark_obtained;?></td>
                            <?php }else{
                            foreach($cos as $coss){?>
                              <td  style="border : 1px solid black; color: #000; text-align : center; "><?php echo $this->crud_model->get_bmarks_average($coss['id'], $id);?></td>
                            <?php }   ?>
                              <td  style="border : 1px solid black; color: #000; text-align : center; "<?php if($this->crud_model->get_stmarks_average($mark['class_id'], $mark['section_id'], $sub['id'], $bids['id'], $mark['student_id'])->row()->mark_obtained < 10) { ?> style="color:red;" <?php } ?>><?php echo 
                    ...
                    foreach($exs as $bids){ ?>
                        ...
                        <td style="border : 1px solid black; color: #000; text-align : center;"><?php echo $this->crud_model->get_subdiv_average($class_id, $section_id, $bids['id'], $mark['student_id'])->row()->total_coef;?></td>
                    ...
                    <?php } ?>
                        <td style="background-color: #ccc;"><?php echo $mark['total_mark'];?></td>
                        <td><?php echo $mark['total_coef'];?></td>
                        <td style="background-color: #ccc;"><?php echo $mark['average'];?></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <table width="100%" style="border-collapse: collapse; font-size:9px">
        ...
        <?php 
            ...
            <tr>
                    ...
                    <td style="border : 1px solid black;"><?php echo $this->user_model->get_user_details($student['user_id'], 'name'); ?></td>
                    ...
                    <?php
                      foreach($exs as $bids){
                        ...
                        $id = $this->crud_model->get_idmarks_average($mark['class_id'], $mark['section_id'], $subject_id, $bids['id'], $mark['student_id']);
                            ...
                            if($co == 1){ ?>
                                <td style="border : 1px solid black; text-align : center;" <?php if($this->crud_model->get_stmarks_average($mark['class_id'], $mark['section_id'], $subject_id, $bids['id'], $mark['student_id'])->row()->mark_obtained < 10) { ?> style="color:red;" <?php } ?>><?php echo $this->crud_model->get_stmarks_average($mark['class_id'], $mark['section_id'], $subject_id, $bids['id'], $mark['student_id'])->row()->mark_obtained;?></td>
                            <?php }else{
                            foreach($cos as $coss){?>
                              <td style="border : 1px solid black; text-align : center;"><?php echo $this->crud_model->get_bmarks_average($coss['id'], $id);?></td>
                            <?php }   ?>
                              <td style="border : 1px solid black; text-align : center;" <?php if($this->crud_model->get_stmarks_average($mark['class_id'], $mark['section_id'], $subject_id, $bids['id'], $mark['student_id'])->row()->mark_obtained < 10) { ?> style="color:red;" <?php } ?>><?php echo $this->crud_model->get_stmarks_average($mark['class_id'], $mark['section_id'], $subject_id, $bids['id'], $mark['student_id'])->row()->mark_obtained;?></td>
                            <?php }   ?>
                    <?php } ?>
                
                    <td style="border : 1px solid black; background-color: #ccc; text-align : center;"><?php echo $this->crud_model->get_subject_average($mark['class_id'], $mark['section_id'], $subject_id, $exam_id, $mark['student_id'])->row()->coef; ?></td>
                    <td style="border : 1px solid black; text-align : center; <?php if($this->crud_model->get_subject_average($mark['class_id'], $mark['section_id'], $subject_id, $exam_id, $mark['student_id'])->row()->mark_obtained < 10) { ?>; color:red; <?php } ?>"><?php echo $this->crud_model->get_subject_average($mark['class_id'], $mark['section_id'], $subject_id, $exam_id, $mark['student_id'])->row()->mark_obtained; ?></td>
