    ...
    <div class="col-12">
        <div class="card">
            <div class="row mt-3">
                ...
                <div class="col-md-2 mb-1">
                    <select name="section" id="section_id" class="form-control select2" data-toggle = "select2" required onchange="classWiseSubject()">
...
function classWiseSubject() {
    ...
    if(exam_id != ""){
    $.ajax({
        url: "<?php echo route('class_wise_subjects/'); ?>"+classId+'/'+section_id+'/'+exam_id,
        success: function(response){
...
//        if(class_id != "" && section_id != "" && exam != ""){
            $.ajax({
                ...
                data : {class_id : class_id, section_id : section_id, exam : exam ,exam_id : exam_id,},
                success : function(response){
                    toastr.success('<?php echo get_phrase('Class_general_average_has_been_updated_successfully'); ?>');
                }
            });
//        }else{
//            toastr.error('<?php echo get_phrase('fill_all_required_field'); ?>');
