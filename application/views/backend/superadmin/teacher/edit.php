  ...
  <form method="POST" class="d-block ajaxForm" action="<?php echo route('teacher/update/'.$param1); ?>">
    <div class="form-row">
      ...
      <div class="col-md-6">
        <label class="form-label" for="bt"><?php echo get_phrase('birth_type'); ?></label>
           <select name="bt" id="bt" class="form-control select" data-toggle = "select2"  required onchange="get_form_val(this.value)">
                <option value="on" <?php if($user['birthtype'] == "on") echo 'selected'; ?>><?php echo get_phrase('born_on'); ?></option>
                <option value="around" <?php if($user['birthtype'] == "around") echo 'selected'; ?>><?php echo get_phrase('born_around'); ?></option>
            </select>
            <small id="" class="form-text text-muted"><?php echo get_phrase('provide_birthtype'); ?></small>
        </div>
        
        <div class="col-md-6" <?php if($user['birthtype'] != "on"){?> style="display:none" <?php } ?> id="bd1">
            <label class="form-label" for="birthdatepicker"><?php echo get_phrase('birthday'); ?></label>
            <input type="text" class="form-control date" id="date" data-toggle="date-picker" data-single-date-picker="true" name = "birthday"   value="<?php if($user['birthtype'] == "on") echo date('m/d/Y', $user['birthday']); ?>">
            <small id="" class="form-text text-muted"><?php echo get_phrase('provide_birthday'); ?></small>
        </div>

        <div class="col-md-6"<?php if($user['birthtype'] != "around"){?> style="display:none" <?php } ?> id="bd">
            <label class="form-label"><?php echo get_phrase('year_of_birth'); ?></label>
            <input type="text" name="birthday1" value="<?php echo $user['birthday'];?>" class="form-control" placeholder="<?php echo get_phrase('year_of_birth'); ?>">  
        ...
        <div class="col-md-6">
            ...
            <input type="text" id="place" name="place" class="form-control" value="<?php echo $user['birthplace'];?>" placeholder="<?php echo get_phrase('place_of_birth'); ?>"> 
      ...
      <div class="form-group col-md-6">
        ...
        <input type="text" value="<?php echo $user['phone']; ?>" class="form-control" id="phone" name = "phone">
      ...
      <div class="form-group col-md-12">
        ...
        <textarea class="form-control" id="about" name = "about" rows="2" ><?php echo $teacher['about']; ?></textarea>
