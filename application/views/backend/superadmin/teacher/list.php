<?php
$school_id = school_id();
$check_data = $this->db->get_where('teachers', array('school_id' => $school_id, 'session' => active_session()));
...
<table id="basic-datatable" class="table table-striped dt-responsive nowrap" width="100%">
    ...
    <tbody>
        <?php
        $c=1;
        $teachers = $this->db->get_where('teachers', array('school_id' => $school_id, 'session' => active_session()))->result_array();
        foreach($teachers as $teacher){
            ?>
            <tr>
                <td><?php echo $c++; ?></td> 
                <td><img class="rounded-circle" width="50" height="50" src="<?php echo $this->user_model->get_user_image($teacher['user_id']); ?>"></td>
                <td><?php echo $this->db->get_where('users', array('id' => $teacher['user_id']))->row('name'); ?><br>
                <em><?php echo $this->db->get_where('users', array('id' => $teacher['user_id']))->row('email'); ?></em></td>
